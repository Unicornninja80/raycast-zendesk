...
export class TicketToArticleService {
  ...
  async getTicketConversation(ticketId: number): Promise<{
    ...
    try {
      ...
      // Get all comments
      const commentsResponse = await zdFetch<{ comments: TicketComment[] }>(`/api/v2/tickets/${ticketId}/comments.json`);
   ...
   */
  async generateArticleFromTicket(
    ticketId: number,
      ...
      const response = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: "You are a technical writing expert who creates clear, comprehensive support articles from customer support conversations."
            },
            {
              role: "user",
              content: prompt
  ...
  private formatConversationForAI(conversation: {
    ...
    comments
      ...
      .forEach((comment, index) => {
        ...
        const roleDisplay = userRole === 'end_user' ? 'CUSTOMER' : 
   ...
   */
  private buildArticleGenerationPrompt(conversationText: string, images: Array<{ url: string; filename: string }>): string {
...
Remember: Create a generic troubleshooting guide that any future person with this problem can follow.`;
   ...
   */
  async createArticleInHelpCenter(articleContent: ArticleContent): Promise<{ article: { id: number; html_url: string; title: string } }> {
    try {
