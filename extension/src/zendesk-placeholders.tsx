...
const placeholderCategories: PlaceholderCategory[] = [
  ...
  },
  {
    name: "Custom Fields",
    placeholders: [
      { placeholder: "{{ticket.ticket_field_[ID]}}", description: "Custom ticket field by ID", example: "{{ticket.ticket_field_123456}}" },
      { placeholder: "{{ticket.ticket_field_option_title_[ID]}}", description: "Custom field option title", example: "{{ticket.ticket_field_option_title_123456}}" }
    ]
  },
  {
    name: "Time & Dates",
      ...
      actions={
        <ActionPanel>
          <Action.CopyToClipboard 
            ...
            content={placeholderCategories.flatMap(cat => 
              cat.placeholders.map(p => p.placeholder)
            ).join('\n')}
          />
        </ActionPanel>
      }
