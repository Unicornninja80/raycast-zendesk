import { Action, ActionPanel, List, showToast, Toast, Form, getPreferenceValues, popToRoot } from "@raycast/api";
...
function CreateArticleForm() {
  ...
  async function loadPermissionGroups() {
    try {
      const data = await zdFetch<PermissionGroupsResponse>("/api/v2/permission_groups.json");
      setPermissionGroups(data.permission_groups);
      
      // Debug: Show available permission groups
      if (data.permission_groups.length > 0) {
        console.log("Available permission groups:", data.permission_groups.map(pg => `${pg.id}: ${pg.name}`));
      }
    } catch (e) {
      // Permission groups might not be available or accessible
      console.log("Could not load permission groups:", e);
      await showToast({ 
        style: Toast.Style.Failure, 
        title: "Could not load permission groups", 
        message: "Will use default ID" 
      });
    }
  }
    ...
    >
      ...
      >
        ...
        {sections.length === 0 && selectedCategoryId && (
          <Form.Dropdown.Item
            value=""
            title="No sections available in this category"
          />
        )}
      </Form.Dropdown>
      ...
      {markdownContent && (
        <Form.Description 
          title="HTML Preview" 
          text={`The markdown will be converted to HTML. First 200 characters:\n\n${convertMarkdownToHTML(markdownContent).substring(0, 200).replace(/<[^>]*>/g, '')}${markdownContent.length > 200 ? '...' : ''}`}
        />
      )}
    </Form>
  );
